...
const MovieItems = (props) => {
    ...
    const moviesAvailable = <div>
        <ul className={styles.items}>
            {props.moviesStored.length > 0 && props.moviesStored.map((movie) => { return (<MovieItem displayMovie={props.displayMovie} title={movie.title} overview={movie.overview} review={movie.review} release={movie.release} img={movie.poster} key={movie.id} id={movie._id}/>) })}
        </ul>
    </div>;
